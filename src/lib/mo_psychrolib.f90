...
module mo_psychrolib
  ...
  function GetTRankineFromTFahrenheit(TFahrenheit) result(TRankine)
    ...
    real(dp), intent(in)  :: TFahrenheit
      !+ Temperature in degree Fahrenheit
    real(dp)              :: TRankine
      !+ Temperature in degree Rankine
    real(dp)              :: ZERO_FAHRENHEIT_AS_RANKINE
  ...
  function GetTKelvinFromTCelsius(TCelsius) result(TKelvin)
    ...
    real(dp), intent(in)  :: TCelsius
      !+ Temperature in degree Celsius
    real(dp)              :: TKelvin
      !+ Tempearatyre in Kelvin
    real(dp)              :: ZERO_CELSIUS_AS_KELVIN
  ...
  function GetTDewPointFromVapPres(TDryBulb, VapPres) result(TDewPoint)
    ...
    !+ TDryBulb is not real(dp)ly needed here, just used for convenience.
    ...
    integer(i4)             :: index
     ...
     real(dp) ::  T_WATER_FREEZE, T_WATER_FREEZE_LOW, T_WATER_FREEZE_HIGH, PWS_FREEZE_LOW, PWS_FREEZE_HIGH
  ...
  function GetTDryBulbFromEnthalpyAndHumRatio(MoistAirEnthalpy, HumRatio) result(TDryBulb)
    ...
    real(dp), intent(in)  ::  MoistAirEnthalpy
  ...
  subroutine CalcPsychrometricsFromTWetBulb(TDryBulb,           &
    ...
    real(dp), intent(out)   ::  DegreeOfSaturation
