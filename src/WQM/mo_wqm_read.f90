...
!  use mo_file,                   only: &
!      file_namelist_param, unamelist_param
  ...
  !======================================================
  call read_wqm_parameters(file_param_wqm)
   
    !===============================================================
    ! Settings for Optimization
    !===============================================================
    ! namelist for Optimization settings
    call position_nml('Optimization', 89)
    read(89, nml=Optimization)
    ! update the number of parameters activated in calibration using sce
    ! after read in the nutrient parameter
  ...
  ! --------------------------------------------------------------------------------
  subroutine read_wqm_parameters(flnm)
  ...
  use mo_mrm_constants,          only: nColPars
  use mo_append,                 only: append
  use mo_string_utils,           only: num2str
  use mo_message,                only: message
  
  character(256), intent(in)            :: flnm
  ...
  ! inquire(file = flnm, exist = fexist)
  ! if (fexist) then
  ! call message('file does exist!!!')
  ! else
  ! call message('not exist...')
  ! end if
  call open_nml(flnm, 99, quiet =.true.)
  !read nitrate submodel parameters
  call position_nml('nutrientparameter', 99)
  read(99, nml=nutrientparameter)
  processMatrix(11,1) = 1_i4                   ! if wqm_config is processing, then this process should be turned on (=1)
  processMatrix(11,2) = 11_i4                   !parameters number introduced              
...
!  call append(global_parameters, reshape(groundwater_residtime,(/1,nColPars/)))
  ...
  end subroutine read_wqm_parameters 
