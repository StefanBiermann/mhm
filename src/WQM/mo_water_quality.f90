...
CONTAINS
  ...
  subroutine wqm(&
      ...
      !routing
      do_rout,           & ! logical if routing active or not
      processm8,         & ! setting value for routing process (No 8): 1-original or 2-adaptive
      tsRoutFactor,      & ! timestep factor (routTS/timestep)
      RunToRout,         & ! accumulative total runoff (adaptive routing)
      cRunToRout,        & ! accumulative conc of total runoff (adaptive routing)
      InflowDischarge,   & ! accumulative inflow/PS (adaptive routing)
      InflowConc,        & ! accumulative inflow/PS conc. (adaptive routing)
      ...
      nNode_qOUT,        & ! outflow from each L11 cell
    ...
    integer(i4),                   intent(in)   :: processm8
    real(dp),                      intent(in)   :: tsRoutFactor
    real(dp), dimension(:),        intent(in)   :: RunToRout
    real(dp), dimension(:,:),      intent(inout):: cRunToRout
    real(dp), dimension(:),        intent(in)   :: InflowDischarge
    real(dp), dimension(:,:),      intent(inout):: InflowConc
    ...
    do k = 1, nCells1
       ...
       !modification corresponding to the adaptive routing
       if ((processm8 .eq. 2) .and. (tsRoutFactor .gt. 1.0_dp)) then
           cRunToRout(k,:) = (cRunToRout(k,:) * (RunToRout(k)- total_runoff(k)) + (total_runoff(k) * ctotal_runoff(k,:))) &
                            / RunToRout(k)
       else
    ...
    !**************************
    ! modification for the adaptive routing
    if ((processm8 .eq. 2) .and. (tsRoutFactor .gt. 1.0_dp)) then
        InflowConc(:,1) = (InflowConc(:,1) * (InflowDischarge(:)- Qinflow(:)) + (Qinflow(:) * cqInflow(:,1))) &
                            / InflowDischarge(:)
        InflowConc(:,2) = (InflowConc(:,2) * (InflowDischarge(:)- Qinflow(:)) + (Qinflow(:) * cqInflow(:,2))) &
                            / InflowDischarge(:)
        RoutTS = timestep * nint(tsRoutFactor, i4)
    else
        InflowConc(:,:) = cqInflow(:,:)
        RoutTS = timestep
    end if
    ...
    if (do_rout) then
        ...
        call conc_routing_accmix(nsubstances, nNodes, RoutTS, RunToRout, &
            cRunToRout,areaCell1,L11id_on_L1, L1id_on_L11, map_flag, nLink_from, reachtemp1,nInflowGauges,  &
            InflowHeadwater, InflowNodeList,InflowIndexList,  InflowDischarge, InflowConc, nLink_rivertemp, nNode_concOUT )

        !in-stream processes
        call conc_routing_process(nNodes, nNodes-1 ,  RoutTS, nPerm, nLink_from, nLink_to, nLink_length, &
  ...
  do k=1, nLinks  
     ...
     call instream_nutrient_processes(TS, rivertemp11(i), L11_rivert_avg10(i), L11_rivert_avg20(i), &
  ...
  subroutine instream_nutrient_processes(TS, rivertemp11, rt_avg10, rt_avg20, beneath_area, depth, &
  ...
  real(dp),                       intent(in)     :: pardeniratew !kg m-2 d-1
  real(dp),                       intent(in)     :: parprodrate  !kg m-3 d-1
